...
#define JANUS_SAMPLEEVH_PACKAGE			"janus.eventhandler.samplekoremsg"
...
int janus_sampleevh_init(const char *config_path) {
	...
	g_print( "*** %s initialized! ***\n", JANUS_SAMPLEEVH_NAME);
	JANUS_LOG(LOG_WARN, "***  %s' INITIALIZED! ***\n", JANUS_SAMPLEEVH_NAME);
	return 0;
}
...
static void *janus_sampleevh_handler(void *data) {
	...
	g_print("Joining SampleEventHandler handler thread\n");
			...
			while(TRUE) {
				...
				switch(type) {
					...
					case JANUS_EVENT_TYPE_TRANSPORT:
						/* This is a transport related event (TODO). The shit you do. No any need here
						 *  The syntax of
		...
		if(res != CURLE_OK) {
			...
			if(max_retransmissions > 0) {
				...
				if(retransmit == max_retransmissions) {
					...
					int next = retransmissions_backoff * 2;//(pow(2, retransmit));
	...
	g_print("LEAVING SAMPLE KORE MSG EVENT HANDLER THREAD!***\n");
	return NULL;
}
