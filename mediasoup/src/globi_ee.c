...
static void ee__on                ( ee_t* self, const char* name, int once, const ee_cb cb, void* eArg);
...
static void ee__on(ee_t* self, const char* name, int once, const ee_cb cb, void* const eArg) {
  ee__event_t *event;
...
void ee_on(ee_t* self, const char* name, const ee_cb handler, void* const eArg) {
  ee__on(self, name, 0, handler, eArg);
}

void ee_add_listener(ee_t* self, const char* name, const ee_cb handler, void* const eArg) {
  ee_on(self, name, handler, eArg);
}

void ee_once(ee_t* self, const char* name, const ee_cb handler, void* const eArg) {
  ee__on(self, name, 1, handler, eArg);
}
...
void ee_emit(ee_t* self, const char* name, void* arg) {
  ...
  while((node = list_iterator_next(it))) {
    ...
    handler->cb(arg, handler->extraArg);
    if (handler->once) {
      list_remove(event->handlers, node);
    }
  }
